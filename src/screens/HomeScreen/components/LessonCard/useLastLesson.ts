...
export const useLastLesson = (): LessonResult => {
  ...
  const progressDataInitialized = useProgressStore(
  ...
  const fetchLesson = useCallback(async () => {
    ...
    try {
      ...
      if (!currentLesson) {
        const allCompleted =
          findIncompleteLessonFromIndex(0, progressData, allStageLessons) ===
          null
        setAllCompleted(allCompleted)
        setLesson(null)
      } else {
        const lessonWithProgress = mergeProgressData(
          currentLesson,
          progressData
        )
...
const findIncompleteLessonFromIndex = (
  ...
  for (let i = startIndex; i < allStageLessons.length; i++) {
    ...
    const lesson =
      getTheoryLessonWithProgress(stageLesson.id, progressData) ?? null
    if (!lesson) continue
...
const findLessonToDisplay = (
  ...
  if (lastAccess) {
    const startIndex = allStageLessons.findIndex(
      (stageLesson) => stageLesson.id === lastAccess.lessonId
    )
    const lessonFromId =
      getTheoryLessonWithProgress(lastAccess.lessonId, progressData) ?? null
    ...
    if (isLessonComplete(lessonFromId, progress)) {
      ...
      const nextLesson = findIncompleteLessonFromIndex(
        startIndex + 1,
        progressData,
        allStageLessons
      )
