...
export const InputField = styled.View<{ isTablet?: boolean; disabled?: boolean }>(({ theme, isTablet, disabled }) => ({
  flexDirection: 'row',
  alignItems: 'center',
  borderWidth: 1,
  borderRadius: scale(12),
...
export const RefreshButton = styled(TouchableOpacity)<{ isTablet?: boolean }>(({ theme, isTablet }) => ({
  ...
  marginTop: isTablet ? scale(10) : scale(12)
}))
...
export const LabelText = styled.Text<{ isTablet?: boolean }>(({ theme, isTablet }) => ({
  fontSize: isTablet ? scale(10) : scale(12),
  color: theme.colors.secondary,
  fontFamily: getSourGummyFontFamily('400'),
  marginBottom: isTablet ? scale(4) : scale(6)
}))
