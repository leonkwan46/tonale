...
import { stagesArray } from '@/subjects/aural/curriculum/stages/helpers'
import { Stage, StageLesson } from '@/subjects/aural/curriculum/types'
...
export const AuralScreenBody = () => {
  ...
  const toggleStageCollapse = useCallback((stageId: string) => {
    ...
    if (willCollapse) {
      ...
      // Small delay to ensure DOM update before starting animation
      setTimeout(() => {
    ...
    <ContentWrapper ref={scrollViewRef} onContentSizeChange={handleContentSizeChange}>
      <ContentContainer>
        ...
        {!hasNextStage && (
          <MessageOverlay>
            <MessageContainer>
              <MessageText>In progress building...</MessageText>
            </MessageContainer>
          </MessageOverlay>
        )}
